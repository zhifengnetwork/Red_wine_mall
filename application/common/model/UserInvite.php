                     ...
                     //这里有个条件前提   周数和业绩   不同要求不一样
                         ...
                         if($periodInfo['begin_time']){  //如果时间已经开始再操作下面
                             ...
                             if($periodInfo['poped_per_num']<$periodInfo['person_num']){ //还有位置就操作
                                ...
                                Db::name('account_log')->insert(['user_id'=>$recommend_id,'user_money'=>$pop_money,'pay_points'=>$addmoney,'change_time'=>$time,'desc'=>"邀请1个新会员【{$myInfo['nickname']}-id:{$myInfo['id']}】奖励50",'type'=>2]);
                                Db::name('account_log')->insert(['user_id'=>$recommend_id,'user_money'=>$minusmoney,'pay_points'=>$minusmoney,'change_time'=>$time,'desc'=>"邀请1个新会员【{$myInfo['nickname']}-id:{$myInfo['id']}】平台扣除手续费{$minusmoney}",'type'=>7]);
