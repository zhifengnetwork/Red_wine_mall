...
class Photoshop extends MobileBase {
    ...
    public function getPosterPhoto($user_id,$card_no){
        $result = [];
        try{
            header('Access-Control-Allow-Origin:*');
            $compay_name = '广州健康商城有限公司';
...
//            $amount_color = '#FF0000';
            ...
            $enter_font_value = '卡号：'.$card_no;
            $amount_size = '24';
            $enter_font_color = '#228B22';
...
//            $qrcode->thumb($width,$height,Image::THUMB_CENTER)->save($qr_code_path);
            ...
            //保存图片到本地
            $save_path = ROOT_PATH .'/public/upload/poster/'.$user_id.'.jpg';

            //贴图贴文字
                    $bg ->text($compay_name,$name_font,$name_size,$name_color,['10','30'])
//                        ->text($user_name,$name_font,$amount_size,$amount_color,['380','310'])
                        ->text($enter_font_value,$name_font,$amount_size,$enter_font_color,['10','310'])
...
//            unlink($radius_avatar_path);
            $return_path = SITE_URL.'/public/upload/poster/'.$user_id.'.jpg';
            return $return_path;
        }catch (Exception $e){
